...
def retrieve_ADCs(df, active_erx, channels, event_num=None):
    ...
    for erx in active_erx:
        for ch in channels:
            col_name = f"eRx{int(erx):02d}_ChData{ch}"
...
def Plot_Single_Link_ADC(data_dict, link_to_plot, erxs, channels, runID, event_num=None):
    ...
    _, _, CM0_erx, CM1_erx, _, _ = retrieve_CMs(df, erxs, channels)
...
if __name__ == "__main__":
    ...
    # --- UPDATED: Channel list now excludes the calibration channel "18" ---
    CHANNELS_TO_PROCESS = []
    CHANNELS_TO_SKIP = [18] # Use integers for comparison
    for i in range(37): # Loop 0 to 36
        if i not in CHANNELS_TO_SKIP:
            CHANNELS_TO_PROCESS.append(f"{i:02d}") # Format as two-digit string
    ...
    # --- UPDATED: Logic for loading/saving pickle files now matches module names ---
    ...
    if PLOT_LINK:
        ...
        Plot_Single_Link_ADC(data_all_modules, PLOT_LINK, ERXS, CHANNELS_TO_PROCESS, RUN_ID, event_num=EVENT_NUM)
    else:
        if any(not df.empty for df in data_all_modules.values()):
            print("Generating plots for all modules...")
            Plot_ADCs(data_all_modules, ERXS, CHANNELS_TO_PROCESS, RUN_ID, event_num=EVENT_NUM)
        else:
